...
taskForm.addEventListener('submit', (e)=> {
    ...
    if (task.value.trim() != "" ){
        ...
        <div class="grid place-items-center">                
          <span>
            <!-- empty box icon -->
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-7 h-7 inline-block checkIcon">
    ...
    checkIcon.forEach((iconElement) => {
      iconElement.addEventListener('click', (pointerEvent) => {

        let taskElement = iconElement.parentElement.parentElement.parentElement
        let todoText = taskElement.firstElementChild.textContent
        ...
        <div class="task">
          <div class="line-through">${todoText}</div>
          <div class="grid place-items-center">                
            <span>
              <!-- check icon -->
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block checkIcon">
    ...
    let trashIcon = document.querySelectorAll(".trashIcon")
      ...
      // trasch icon delete task functionality
      trashIcon.forEach((iconElement) => {
        iconElement.addEventListener('click', (pointerEvent) => {
            let taskElement = iconElement.parentElement.parentElement.parentElement
            taskElement.remove()
